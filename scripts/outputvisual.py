    ...
    for fn in path_list:
        ...
        with open('../tmp/tmp.txt', 'r') as f:
            ...
            for line in lines:
                ...
                img_output = cv2.imread(img_paths, 0)
                plt.subplot(1,3,i)
                i += 1
                plt.imshow(img_output,"gray")
                if(hh.isBlack(img_output) == 1):
                    plt.title('Black')
                else:
                    plt.title(outputsort(sess.run(recog, feed_dict={x1: outputdata, y1: np.ones([1, 4096], dtype='float')})))
            plt.show()
